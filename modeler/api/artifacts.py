...
class Artifacts(IEXClient):
    ...
    def get_artifact(self, artifact):
        ...
        if artifact == 'analyst':
            return pd.DataFrame([{**{'dataDate':time_ordinal()},**{k:v for k,v in {**self.get_price_target(), **self.get_recommendation_trends()[0], **self.get_estimates()}.items() if k in self.fields['analyst']}}]) if self.df is None else pd.concat([pd.DataFrame([{**{'dataDate':time_ordinal()},**{k:v for k,v in {**self.get_price_target(), **self.get_recommendation_trends()[0], **self.get_estimates()}.items() if k in self.fields['analyst']}}]),self.df])
