...
class ClientMethods(object):
    ...
    def fetch_option_positions(cls, client):
        positions = {option['option'].split('/')[:-1][-1]: {k: v for k, v in option.items() if k in ['option','average_price','chain_symbol','type','id','quantity','intraday_quantity']} for option in list(filter(lambda p: float(p["quantity"]) > 0.0, client.get(option_positions())['results']))}
    ...
    @classmethod
    def replace_option_order(cls, client, cancel_url, direction, legs, price, quantity, time_in_force='gfd', trigger='immediate', order_type='limit'):
        result = cls.cancel_option_order(client, cancel_url)
        if result:
            payload = {'account': account(client.account_id), 'direction': direction, 'legs': legs, 'price': str(price), 'quantity': int(quantity), 'time_in_force': time_in_force, 'trigger': trigger, 'type': order_type, 'override_day_trade_checks': False, 'override_dtbp_checks': False, 'ref_id': str(uuid.uuid4())}
            return client.post(option_orders(), payload=payload)
        return False
